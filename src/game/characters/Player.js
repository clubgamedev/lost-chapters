...
export class Player extends Phaser.Sprite {
    constructor(game, x, y) {
        super(game, x * 16, y * 16, "atlas", "idle/hero-idle-back/hero-idle-back");
        this.type = "player";
        this.initX = x * 16;
        this.initY = y * 16;
        this.health = 3;
    ...
    update() {
        ...
        if (this.action == PLAYER_ACTION.PREPARE_ATTACK) {
            if (this.state == PLAYER_STATE.WALKING_DOWN || this.state == PLAYER_STATE.DOWN) {
                this.animations.play("attack-front");
            } else if (this.state == PLAYER_STATE.WALKING_UP || this.state == PLAYER_STATE.UP) {
                this.animations.play("attack-back");
            } else if (this.state == PLAYER_STATE.WALKING_LEFT || this.state == PLAYER_STATE.LEFT) {
                this.animations.play("attack-side");
            } else if (this.state == PLAYER_STATE.WALKING_RIGHT || this.state == PLAYER_STATE.RIGHT) {
                this.animations.play("attack-side");
            }
            return;
        }
