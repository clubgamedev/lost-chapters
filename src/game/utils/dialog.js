...
export function startChoice(choice) {
	...
	let bgSprite = game.add.sprite(
		game.width - dialogWidth - 4,
		topY,
		"dialog-choice-box"
	)
	bgSprite.width = 178
	...
	let textSprite = game.add.text(
		game.width - dialogWidth + 12,
		topY + 4,
	...
	let selectionSprite = game.add.text(
		game.width - dialogWidth,
		topY + 4,
		"►",
		{
			font: "12px Alagard",
...
export function selectChoice() {
	...
	game.dialog.choice.selectionSprite.cameraOffset.y =
		game.height - 40 - 14 * (nbOptions - game.dialog.selectedChoice)
}
...
export function exhaustDialog(questions, endDialogChoice, questionsDone = []) {
	...
	// pass to next line
	if (endDialogChoice) choice[endDialogChoice] = () => []
	else if (Object.keys(choice).length === 0) return []
